...
if(!function_exists('webconfig')){
    ...
    function webconfig($name = null){
        ...
        $list = ['system', 'user', 'web', 'general', "ecommerce", 'mailer', 'payments', 'edu'];
...
if(!function_exists('get_edu_config')){
    /**
     * lấy thông tin cấu hình chung
     *
     * @param string $key
     * @return mixed
     */
    function get_edu_config($key = null, $default = null){
        if($general = webconfig('edu')){
            if(!is_null($key)) return $general->get($key, $default);
            return $general;
        }
        return null;
