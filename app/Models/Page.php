...
class Page extends BasePost
{
    ...
    public $fillable = [
        ...
        'featured_image', 'views', 'privacy', 'trashed_status'
    ];
    ...
    public function getChildren($trashed_status = null)
    {
        $query = self::where('parent_id',$this->id);
        if(!is_null($trashed_status)){
            $query->where('trashed_status',$trashed_status); 
        }
        return $query->get();
    }
    /**
