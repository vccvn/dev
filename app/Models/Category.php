...
class Category extends Model
{
    ...
    public $fillable = ['dynamic_id', 'parent_id', 'name', 'type', 'slug', 'keywords', 'description', 'featured_image', 'trashed_status'];
    ...
    // {
    //     return $this->refPosts()->join('posts', 'posts.id', '=', 'category_refs.ref_id')->where('posts.trashed_status', static::UNTRASHED)->select('category_refs.ref_id', 'category_refs.category_id', 'posts.*');
    // }

    /**
     * The untrashedPosts that belong to the Category
     *
     * @return \Illuminate\Database\Eloquent\Relations\BelongsToMany
     */
    public function untrashedPosts(): BelongsToMany
    {
        return $this->belongsToMany(Post::class, 'category_refs', 'category_id', 'ref_id')->wherePivot('ref', '=', 'post');
    }
