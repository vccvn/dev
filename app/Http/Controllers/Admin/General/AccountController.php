...
class AccountController extends AdminController
{
    ...
    public function done(Request $request, Arr $data)
    {
        ...
        if($this->tab == 'info'){
            ...
            $this->uploadImageAttachFile($request, $data, 'avatar', get_content_path($request->user()->secret_id.'/avatar'), 400, 400);

        }
        if($this->repository->update($request->user()->id, $data->all())){
            return redirect()->route($this->redirectRoute)->with('success', 'admin.account.update.'.$this->tab);
        }
