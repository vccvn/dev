...
class PostController extends AdminController
{
    ...
    /**
     * @var \App\Repositories\Posts\CategoryRepository $categoryRepository
     * Quản lý danh mục
     */
    ...
    protected function beforeSave(Request $request, $data)
    {
        ...
        $this->uploadImageAttachFile($request, $data, 'featured_image', get_content_path('posts'), $this->thumbWidth, $this->thumbHeight);

        // nếu không có ảnh chính
        if (!$data->featured_image && ($this->dynamic->post_type == 'video_embed' || $request->content_type == 'video_embed') && $video = get_video_from_url($data->video_url)) {
            if ($filename = $this->repository->saveFeatureImageFromUrl($video->thumbnail)) {
                $data->featured_image = $filename;
                $image = new Image(public_path(get_content_path('posts/' . $data->featured_image)));
                $image->resizeAndCrop($this->thumbWidth, $this->thumbHeight);
                $image->save(public_path(get_content_path('posts/thumbs/' . $data->featured_image)));
            }
        }

        $this->makeSocialImage($data, get_content_path('posts'));
    }
    ...
    public function afterSave(Request $request, $result, $data)
    {
        ...
        $this->categoryRefRepository->updateCategoryRef('post', $result->id, $this->categoryRepository->getTree($result->category_id));
