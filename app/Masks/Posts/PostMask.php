...
class PostMask extends Mask
{
    ...
    {
        $this->allow([
            'getViewUrl', 'getFullTitle', 'getSeoKeywords', 'getFeaturedImage', 'getThumbnail', 'getCategory', 'getAuthor',
    ...
    public function next($sameCategory = true)
    {
        ...
        return app(PostRepository::class)->notTrashed()
            ->mode('mask')
            ->where('id', '>', $this->id)
            ->orderBy('id', 'ASC')
            ->detail($params);
    ...
    public function getComments($args = null)
    {
        ...
        elseif(is_array($args)){
            if(array_key_exists('page_size', $args)){
                if(is_numeric($args['page_size'])) $pageSize = $args['page_size'];
                unset($args['page_size']);
            }
            if(array_key_exists('pageSize', $args)){
                if(is_numeric($args['pageSize'])) $pageSize = $args['pageSize'];
                unset($args['pageSize']);
            }
        ...
        return (new CommentRepository())->paginate($pageSize)->mode('mask')->getData($params);
    }
}
