...
class TagRepository extends BaseRepository
{
    ...
    public function getHaveProducts($args = [])
    {
        $this->withCount('refProducts as product_count')->mode('mask')->havingRaw('product_count > 0')->get($args);
    }
    ...
    public function createTag($name)
    {
        ...
        if(!$keyword || strlen($name) > Tag::MAX_LENGTH) return null;
    ...
    {
        if(!$this->find($id) || strlen($name) > Tag::MAX_LENGTH) return false;
        $a = null;
        $keyword = vnclean($name);
        $name_lower = vntolower($name);
        $slug = str_slug($keyword);
        ...
        if($tags){
            if(count($tag_list = explode(',', $tags))){
                foreach ($tag_list as $t) {
                    if(strlen($t) > Tag::MAX_LENGTH){
                        $this->errors[] = "\"$t\" Vượt quá ".(Tag::MAX_LENGTH)." ký tự";
                    }
                    elseif($ta = $this->findBy('name_lower',trim(vntolower($t)))){
                        $a[] = $ta;
     ...
     * @return array
     */
    public function getRefTagOptions(string $ref = 'post', $ref_id = 0, array $args = [])
    {
        $b = [];
        if($ref){
            $b['ref'] = $ref;
