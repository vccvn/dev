...
use App\Validators\Posts\CategoryValidator;
...
class CategoryRepository extends BaseRepository
{
    ...
    public function beforeGetData($args = [])
    {
        ...
        if (isset($args['@count_post'])) {
            $this->joinPosts = true;
            $this->withCount('untrashedPosts as post_count');
        }
    ...
    public function avaliableCategory()
    {
        $this->join('dynamics', 'dynamics.id', '=', 'categories.dynamic_id')
            ->where('dynamics.trashed_status', 0)
    ...
    public function getCategories($args = [])
    {
        ...
        if (isset($args['@advance']) && is_array($args['@advance']) && in_array('post_count', $args['@advance'])) {
            ...
            $this->withCount('untrashedPosts as post_count')->with(['children' => function ($query) {
                $query->withCount('untrashedPosts as post_count')->with(['children' => function ($query) {
                    $query->withCount('untrashedPosts as post_count')->with(['children' => function ($query) {
                        $query->withCount('untrashedPosts as post_count');
                    }]);
                }]);
            }]);
        }
