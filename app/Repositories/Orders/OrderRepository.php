...
class OrderRepository extends BaseRepository
{
    ...
    public function updateStatusOnly($id, int $status = 0)
    {
        ...
        if ($id && ($old = $this->with(['items' => function($q){$q->with('itemProduct');}])->detail($id)) && ($detail = $this->update($id, ['status' => $status]))) {
            if($old->delivery_status == Order::DELIVERY_PENDING && $status == Order::CANCELED){
                if($old->items && count($old->items)){
                    foreach ($old->items as $item) {
                        if($product = $item->itemProduct){
                            $product->available_in_store = $product->available_in_store + $item->quantity;
                            $product->save();
                        }
                    }
                }
