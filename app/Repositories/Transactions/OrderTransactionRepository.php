...
use App\Masks\Transactions\TransactionCollection;
use App\Masks\Transactions\TransactionMask;
use App\Models\Order;
use App\Models\Transaction;
use Gomee\Repositories\BaseRepository;
use App\Repositories\Customers\CustomerRepository;
use App\Repositories\Orders\OrderRepository;
use App\Validators\Transactions\OrderTransactionValidator;
...
class OrderTransactionRepository extends BaseRepository
{
     ...
     */
    protected $validatorClass = OrderTransactionValidator::class;
    ...
    public function init()
    {
        ...
        $this->setJoinable([
            ...
            ['join', 'orders', 'orders.id', '=', 'transactions.ref_id']
        ]);
        $columns = [
        ...
        if ($id && $transaction = $this->detail($args)) {
            ...
            if ($transaction->status != $status && ($detail = $this->update($id, ['status' => $status]))) {
                ...
                if ($status == 1) {
                    ...
                    if ($order && $order->payment_status > Order::PAYMENT_PENDING_VERIFY) {
