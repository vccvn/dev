...
class Breadcrumb
{
    ...
    protected function _append($text = null, $url = null)
    {
        $type = $url ? 'url' : 'text';
        $fullText = $text;
        $index = count(static::$data);
        static::$data[] = new Arr(compact('type', 'text', 'fullText', 'url'));
    ...
    protected function _prepend($text = null, $url = null)
    {
        $type = $url ? 'url' : 'text';
        $fullText = $text;
        array_unshift(static::$data, new Arr(compact('type', 'text', 'fullText', 'url')));
    ...
    {
        $tree = $page->getTree();
        if ($tree && is_countable($tree) && count($tree)) {
            foreach ($tree as $cate) {
    ...
    protected function _addproduct($product)
    {
        ...
        $this->_append($product->sku, $product->getViewUrl())->fullText = $product->name;
        return $this;
    }
    ...
    protected function _getfulltitle()
    {
        ...
        foreach (static::$data as $key => $item) {
            array_unshift($title, $item->fullText);
        }
        if ($title) {
    ...
    {
        $a = static::$data;
        array_unshift($a, new Arr(['type' => 'url', 'text' => siteinfo('site_name', 'Trang chủ'), 'fullText' => siteinfo('site_name', 'Trang chủ'), 'url' => route('home')]));
        return $a;
    }
