...
class MenuEngine
{
    ...
    public function get(string $filename)
    {
        ...
        $menuitems = isset($data['dashboard']) && (($user && $user->type == 'admin') || (
            (array_key_exists('permiss', $data['dashboard']) && PermissionService::checkModulePermiss($data['dashboard']['permiss'], $roles)) ||
            (array_key_exists('route', $data['dashboard']) && PermissionService::checkModulePermiss($data['dashboard']['route'], $roles))
        ))? [$data['dashboard']] : [];
        $issetPost = false;
        // dd($data);
        if (count($itemList)) {
            foreach ($data as $key => $item) {
                ...
                if (in_array($key, $itemList)) {
                    ...
                    elseif ($cond1 && !PermissionService::checkModulePermiss($item['permiss'], $roles))
                        continue;
                    elseif ($cond2 && !PermissionService::checkModulePermiss($item['route'], $roles))
                        continue;
                    elseif(!$cond1 && !$cond2)
                        continue;
                        ...
                        foreach ($sData as $index => $sub) {
                            ...
                            if(
                                ($user && $user->type == 'admin') || (
                                    (array_key_exists('permiss', $sub) && PermissionService::checkModulePermiss($sub['permiss'], $roles)) ||
                                    (array_key_exists('route', $sub) && PermissionService::checkModulePermiss($sub['route'], $roles))
                                )
                            ){}
                            else{
                    ...
                    if (is_array($item)) {
                        foreach ($item as $custom) {
                            if ($custom == 'posts' && (($user && $user->type == 'admin') || PermissionService::checkModulePermiss('admin.posts', $roles))) {
                                $menuitems = array_merge($menuitems, $this->getPostMenuItems($user, $roles));
                                $issetPost = true;
                            } else {
                            }
                        }
                    } elseif ($item == 'posts' && (($user && $user->type == 'admin') || PermissionService::checkModulePermiss('admin.posts', $roles))) {
                        $menuitems = array_merge($menuitems, $this->getPostMenuItems($user, $roles));
                        $issetPost = true;
                    }
                    ...
                    if ($packageMenus) {
                        foreach ($packageMenus as $key => $value) {
                            if (
                                ($user && $user->type == 'admin') || 
                                (array_key_exists('permiss', $value) && PermissionService::checkModulePermiss($value['permiss'], $roles)) || 
                                (array_key_exists('route', $value) && PermissionService::checkModulePermiss($value['route'], $roles))
                            ) {
                                $menuitems[] = $value;
                            }
                        }
            ...
            if (!$issetPost && (($user && $user->type == 'admin') || PermissionService::checkModulePermiss('admin.posts', $roles))) {
                $menuitems = array_merge($menuitems, $this->getPostMenuItems($user, $roles));
            }
        }
        $sidebar_menu = [
        ...
        if (count($dynamics = $dynamicRepository->notTrashed()->get())) {
            foreach ($dynamics as $item) {
                ...
                if(($user && $user->type == 'admin')){
                    ...
                    if(PermissionService::checkModulePermiss('admin.posts.list', $roles)){
                        $sub_menu[] = [
                            'text' => "Danh sách",
                            'active_key' => 'list',
                            'url' => admin_dynamic_url('list', ['dynamic' => $item->slug]),
                    ...
                    if(PermissionService::checkModulePermiss('admin.posts.create', $roles)){
                        $sub_menu[] = [
                            'text' => "Thêm " . $item->name,
                            'active_key' => 'create',
                            'url' => admin_dynamic_url('create', ['dynamic' => $item->slug]),
                    ...
                    if($item->use_category && PermissionService::checkModulePermiss('admin.posts.categories.list', $roles)){
                        $sub_menu[] = [
                            'text' => "Danh mục",
                            'active_key' => $item->slug . '.categories.list',
                            'url' => admin_dynamic_url('categories.list', ['dynamic' => $item->slug]),
