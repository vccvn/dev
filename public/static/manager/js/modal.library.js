    ...
    var mDropzoneLibrary = {
        init: function () {
            var dropzoneLibrary = new Dropzone("#dropzone-library", {
                ...
                addRemoveLinks: !0,
                renameFile: true,
                acceptedFiles: "image/jpeg,image/gif,image/png,application/svg+xml,*.svg,.svg,image/svg+xml",
                accept: function (e, o) {
                    var n = e.name;
                    var ext = n.split(".").pop();
                    var ex = String(ext).toLowerCase();
                    if (['jpg', 'jpeg', 'png', 'gif', 'svg'].indexOf(ex) == -1) o("Định dạng không được hỗ trợ");
                    else o();
                }
            });
        ...
        if ($inputMedia.length) {
            ...
            App.library.open({
                ...
                selected: $hidden.val(),
                    ...
                    var onc = $inputMedia.data('changed-callback') ?? $inputMedia.data('on-change');
                    if (onc) {
                        let oncbs = onc.split(',');
                        if (oncbs.length > 1) {
                            oncbs.forEach(element => {
                                ...
                                if (App.func.check(func)) {
                                    App.func.call(func, [file, $inputMedia]);
                                }
                            });

                        }
                        else if (App.func.check(onc)) {
                            App.func.call(onc, [file, $inputMedia]);
                        }
                    }
