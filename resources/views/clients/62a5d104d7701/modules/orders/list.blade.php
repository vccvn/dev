            ...
            <div class="row">
                <div class="col-lg-3">
                    <ul class="nav nav-tabs custome-nav-tabs flex-column category-option">
                        ...
                        @endforeach
                        <li class="nav-item">
                            <a class="nav-link font-light {{$key?'': 'active'}}" href="{{route('client.customers.logout')}}"><i class="fas fa-exit"></i> Thoát</a>
                        </li>
                                ...
                                <table class="table cart-table">
                                    ...
                                    @foreach ($orders as $order)
                                        <tr class="{{parse_classname('order-item', 'order-item-'.$order->id)}}" id="{{'order-item-'.$order->id}}">
                                            ...
                                            <td class="wide-column"><p class="mt-0">{{$order->getPaymentMethodText()}}</p></td>
                                            <td><p class="mt-0">{{$order->getStatusLabel()}}</p></td>
