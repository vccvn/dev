    ...
    <div class="main-navbar">
        <div id="mainnav">
            ...
            {!! 
                            ...
                            if($item->hasClass('personal-style-item')){
                                if(count($personalStyles = get_personal_style_sets(['@limit' => 10]))){
                                    ...
                                    foreach ($personalStyles as $style) {
                                        $a .= '<li><a href="'.route('client.style-sets.update', ['id' => $style->id]).'"> <img src="'.theme_asset('images/gif/new.gif').'" />'.$style->name.'</a></li>';
                                    }
                                    $item->sub->prepend($a);
                                }
                            }
                ...
                ')->append(Auth::user()?'': '<li class="mobile-login">
                    <div class="text-center my-auto" style="max-width:40%; margin: 0 auto">
                        <a class="btn btn-theme btn-colored-default" href="'.route('client.account.login').'">Đăng nhập</a>
                    </div>
                </li>')
            !!}
        </div>
